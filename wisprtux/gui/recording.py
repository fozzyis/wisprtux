...
# pylint: disable=import-error, wrong-import-position, too-many-instance-attributes
...
class RecordingView(Gtk.Box):
    ...
    def _update_target_label(self):
        ...
        origin = self.engine.origin_window
        if self.engine.auto_type and origin and origin.valid and origin.window_name:
    ...
    def set_status(self, status):
        """Update the status indicator label and icon"""
    ...
    def append_transcript(self, result):
        """Append new transcription text to the display"""
        if self._has_placeholder:
            self.text_buffer.set_text("")
            self._has_placeholder = False
